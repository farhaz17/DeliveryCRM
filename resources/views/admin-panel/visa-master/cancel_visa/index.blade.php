...
@section('content')
    <div class="breadcrumb">
        ...
        <ul>
            <li><a href=""> Cancel Visa Proces</a></li>
                            ...
                            <div class="row">
                                <div class="col-md-12 form-group mb-3">
                                    ...
                                    <div class="input-group ">
                                        ...
                                        <input class="form-control form-control-sm  typeahead" id="keyword" readonly value="{{ isset($_GET['passport_id']) ? $_GET['passport_id'] : '' }}" autocomplete="off" type="text" required  name="name" placeholder="search..." aria-label="Username" aria-describedby="basic-addon1">
...
<script>
  $(document).on('click', '.sugg-drop', function(){
                    ...
                    $.ajax({
                        url: "{{ route('cancel_visa_process_names') }}",
                        method: 'POST',
                        dataType: 'json',
...
<script>
    function visaCancelTyping(id)
        {
            var id=id;
            var token = $("input[name='_token']").val();
            $.ajax({
                        url: "{{ route('cancel_visa_contract_typing') }}",
                        method: 'POST',
                        dataType: 'json',
...
<script>
    function visaCancelSub(id)
        {
            var id=id;
            var token = $("input[name='_token']").val();
            $.ajax({
                        url: "{{ route('cancel_visa_contract_sub') }}",
                        method: 'POST',
                        dataType: 'json',
                        data:{id:id,_token:token},
                        beforeSend: function () {
...
<script>
    function visaCancel(id)
        {
            var id=id;
            var token = $("input[name='_token']").val();
            $.ajax({
                        url: "{{ route('cancel_visa_contract') }}",
                        method: 'POST',
                        dataType: 'json',
                        data:{id:id,_token:token},
                        beforeSend: function () {
...
<script>
    function visaCancelApproval(id)
        {
            var id=id;
            var token = $("input[name='_token']").val();
            $.ajax({
                        url: "{{ route('cancel_visa_approval') }}",
                        method: 'POST',
                        dataType: 'json',
                        data:{id:id,_token:token},
                        beforeSend: function () {
...
<script>
    function visaCancelDecline(id)
        {
            var id=id;
            var token = $("input[name='_token']").val();
            $.ajax({
                        url: "{{ route('cancel_visa_decline') }}",
                        method: 'POST',
                        dataType: 'json',
                        data:{id:id,_token:token},
                        beforeSend: function () {
