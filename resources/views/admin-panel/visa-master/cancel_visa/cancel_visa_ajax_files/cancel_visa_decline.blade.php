...
<h5 class="modal-title" id="exampleModalCenterTitle">Cancel Visa Decline</h5>
...
<div class="row">
    ...
    @if(isset($cancel_approval) && $cancel_approval->decline_status == '1')
        <div class="col-md-12">

            <label for="repair_category">Status</label>
            @if(isset($cancel_decline))
                <select id="decline_status" name="decline_status" class="form-control form-control"
                  required {{isset($cancel_decline)?'disabled':""}} >
                    @php
                        $isSelected= $cancel_decline->decline_status;
                    @endphp
                    @if($isSelected=='1')
            ...
            @else
                <select id="decline_status" @if ($cancel_decline !=null) disabled
    ...
    <div class="col-md-12" id="attachment" @if(isset($cancel_decline) && $cancel_decline->attachment)  @else style="display: none" @endif >
        <label for="repair_category"> Attachment</label>
        @if(isset($cancel_decline))
            @if($cancel_decline->attachment!=null)
            <br>
            @foreach (json_decode($cancel_decline->attachment) as $visa_attach)
            <a class="btn btn-success btn-file" href="{{Storage::temporaryUrl('assets/upload/visa_cencel/visa_cancel_decline/'.$visa_attach, now()->addMinutes(5))}}"  target="_blank">
...
<div class="row payment_row"  id="payment1" style="display: none">
    <div class="col-md-6">
        ...
        <input class="form-control form-control" id="payment_amount" name="payment_amount"
               type="text" @if ($cancel_decline !=null) readonly

               @endif  value="{{isset($cancel_decline)?$cancel_decline->payment_amount:""}}" placeholder="Payment Amount" />
    </div>
    <div class="col-md-6">
        <label for="repair_category">Payment Type</label>
        @if(isset($cancel_decline))
            <select id="payment_type" name="payment_type" class="form-control form-control"  required {{isset($cancel_decline)?'disabled':""}} >
                @php
                    $isSelected=(isset($cancel_decline)?$cancel_decline->payment_type:"")==$cancel_decline->id;
                @endphp
                <option value="{{$cancel_decline->payment_type}}">{{isset($cancel_decline->payment->payment_type)?$cancel_decline->payment->payment_type:""}}</option>
            </select>
        @else
            <select id="payment_type" @if ($cancel_decline !=null) disabled
    ...
    <div class="col-md-6">
        ...
        <input class="form-control form-control" id="transaction_no" name="transaction_no" @if ($cancel_decline !=null) readonly

        @endif
               type="text" value="{{isset($cancel_decline)?$cancel_decline->transaction_number:""}}" placeholder="Enter Country Code" />
    </div>
    <div class="col-md-6">
        ...
        @if ($cancel_decline !=null) readonly

        @endif  type="date" value="{{isset($cancel_decline)?$cancel_decline->transaction_date:""}}" placeholder="Enter Transaction Date"  />
        <div id="datetime-1-holder"></div>

                                                 </div>
    <div class="col-md-6">
        <label for="repair_category">Vat</label>
        <input class="form-control form-control" id="vat" name="vat"  type="number" value="{{isset($cancel_decline)?$cancel_decline->vat:""}}"
               @if(isset($cancel_decline)) readonly @endif placeholder="Enter VAT"  />
    </div>
    ...
    <div class="col-md-6">
        <label for="repair_category"> Attachment</label>
        @if(isset($cancel_decline))
        @if($cancel_decline->other_attachment!=null)
            <br>
            @foreach (json_decode($cancel_decline->other_attachment) as $visa_attach2)
            <a class="btn btn-success btn-file"  href="{{Storage::temporaryUrl('assets/upload/visa_cencel/visa_cancel_decline/other_attachments/'.$visa_attach2, now()->addMinutes(5))}}"
                 target="_blank">
                 <strong style="color: #000000">  View Attachment</strong>
            </a>
                <span>|</span>
        ...
        <div class="col-md-6 mt-2 form-group">
            ...
            <input @if(isset($cancel_decline)) disabled @endif type="submit" name="btn" value="Save" id="submitBtn" data-toggle="modal" data-target="#confirm-submit" class="btn btn-primary btn-save" />
        </div>
    {{-- @endif --}}
</form>
...
$("#cancel_typing_form").on('submit',(function(e){
    ...
    $.ajax({
        ...
        url: "{{ route('cancel_visa_decline_save') }}",
        type: "POST",
