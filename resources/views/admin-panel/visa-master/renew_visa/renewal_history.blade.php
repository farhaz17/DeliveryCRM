...
@section('css')
    ...
    <style>
       ...
       padding: 1px;
        ...
        a.btn.btn-success.btn-s.btn-icon.m-1 {
    padding: 1px;
}
            ...
            <div class="card">
                <div class="card-body">
                    <table class="table table-striped" id="datatable">
                        ...
                        <tr class="t-row">
                            <th scope="col">Renewal Times</th>
                            <th scope="col">Passport No</th>
                            <th scope="col">PPUID</th>
                            <th scope="col">Name</th>
                            <th scope="col">Renewed At</th>
                            <th scope="col">Expires At</th>
                            {{-- <th scope="col">Renewed Pending</th> --}}
                        ...
                        @foreach($data as $res)
                                <tr>
                                    <td>
                                        <span class="badge badge-square-secondary m-1 font-weight_bold">
                                             {{$res['renewal_times']}}
                                            </span>
                                    </td>
                                    <td>{{$res['pass_no']}}</td>
                                    <td>{{$res['pp_uid']}}</td>
                                    <td>{{$res['name']}}</td>
                                    <td>{{$res['renewed_at']}}</td>
                                    <td>{{$res['expires_at']}}</td>
                                    <td>
                                        <a  class="btn btn-primary btn-sm start-visa"
                                         href="renew_visa_process?passport_id={{$res['pass_no']}}&agreed_id={{$res['id']}}"
                                          target="_blank">
                                          Start Visa Process
                                        </a>
                                    </td>
