...
<div class="card-body">
    <div class="card-title mb-3 col-12">Vehicle Category Reports</div>
    <div class="row">
        <table class="table table-sm table-hover table-striped text-11 data_table_cls" >
            ...
            <tbody>
                @forelse ($vehicle_sub_categories as $category)
                <tr>
                    <td>&nbsp;</td>
                    <td> {{ $category->parent_category->name ?? '' }} </td>
                    <td> {{ $category->name ?? '' }} </td>
                    <td>
                        <a class="text-success mr-2" href="{{ route('vehicle_sub_category.edit', $category->id) }}">
