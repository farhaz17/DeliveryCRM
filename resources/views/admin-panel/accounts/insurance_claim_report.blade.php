...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('vehicle_wise_dashboard',['active'=>'reports-menu-items']) }}">LPO Report</a></li>
      <li class="breadcrumb-item active" aria-current="page">ReportClaim Insurance</li>
    </ol>
</nav>
        ...
        <tbody>
            @foreach ($report as $row)
                <tr>
                    ...
                    <td>@if($row->payment_attachment)
                        @foreach (json_decode($row->payment_attachment) as $doc)
                        <a href="{{Storage::temporaryUrl($doc , now()->addMinutes(60)) }}" target="_blank">View File</a>
                        @endforeach
                    @endif</td>
                    <td>{{ $row->claim_remarks }}</td>
                </tr>
            @endforeach
