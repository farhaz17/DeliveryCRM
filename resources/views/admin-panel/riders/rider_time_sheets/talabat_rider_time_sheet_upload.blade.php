...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Talabat Time Sheet Sheet Upload</li>
        </ol>
    </nav>
        ...
        <div class="col-md-12">
            @if(Session::has('message'))
                ...
                @php
                    ...
                    $missing_city_names = array_filter(explode(',' ,session()->get('missing_city_names')));
                @endphp
                                        ...
                                        <tbody>
                                            @foreach ( $missing_rider_ids as $key => $missing_rider_id)
                                                <tr>
                                                    ...
                                                    <td>{{ $missing_city_names[$key] ?? "NA" }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="post" enctype="multipart/form-data" action="{{ route('talabat_rider_time_sheets.store') }}"  aria-label="{{ __('Upload') }}" >
                        @csrf
                        ...
                        <div class="col-md-3 form-group"  style="float: left;" >
                            <a href="{{ asset('assets/sample/time_sheets/talabat_time_sheet_sample.xlsx') }}" class="float-right" target="_blank">( Download Sample File )</a>
                        </div>
                        <div class="col-md-12 form-group mb-3">
                            <button class="btn btn-sm upload_or_delete_btn btn-info float-right" name="upload_or_delete" type="submit" value="upload">Upload</button>
                            <button class="btn btn-sm upload_or_delete_btn btn-danger float-right mr-2" onclick="return confirm('Are you sure to delete the date cod?')" name="upload_or_delete" id="upload_or_delete" type="submit" value="delete">Delete</button>
                        </div>
                    </form>
                </div>
...
@section('js')
    ...
    <script>
        $('.upload_or_delete_btn').on('click', function(){
