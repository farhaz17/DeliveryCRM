...
<div class="col-md-12 mb-3">
    <div class="card text-left">
        <div class="card-body">
            ...
            <div class="row" id="cash_cod_holder"></div>
        </div>
    </div>
</div>
...
@section('js')
    ...
    <script>
        $(document).on('click','.alert-confirm',function (e) {
            ...
            swal('Deleted!', 'Carrefour Cash Cod has been deleted.', 'success').then(function(){
                  $('#' + form_id).submit();
              });
          }, function (dismiss) {
          // dismiss can be 'overlay', 'cancel', 'close', 'esc', 'timer'
          if (dismiss === 'cancel') {
              swal('Cancelled', 'Cash cod is not deleted :)', 'error');
          }
          });
      });
  </script>
    ...
    <script>
        ...
        $("#apply_filter").click(function () {
            ...
            if(start_date == '' || end_date == ""){
                ...
                $.ajax({
                    url: "{{ route('carrefour_cash_cod') }}",
                    method: 'GET',
                    data: { start_date, end_date },
                    success: function(response) {
                        $('#total_amount').text(response.total_amount)
                        $("#cash_cod_holder").empty(response);
                        $("#cash_cod_holder").append(response.html);
                        selectRefresh();
                        // $('#remove_apply_filter').removeClass('d-none');
                    }
                });
            }
    ...
    <script>
        function selectRefresh() {
            $('#cash_cod_holder .select2').select2({
                tags: true,
