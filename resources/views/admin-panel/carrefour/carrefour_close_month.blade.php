...
<div class="col-md-12 mb-3">
    <div class="card text-left">
        <div class="card-body">
            <form action="{{ route('carrefour_save_close_month') }}" method="POST">
                @csrf
                <div class="row">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <label class="checkbox checkbox-outline-primary">
                                                <input type="checkbox" name="details[{{$key}}][rider_ids]" value="{{ isset($upload->rider_id) ? $upload->rider_id : '' }}"  checked="checked" class="checkbox_cls"><span></span><span class="checkmark"></span>
                                            </label>
                                        </td>
                                        <td>{{$upload->passport->personal_info->full_name}}</td>
                                        <td>{{$upload->passport->carrefour_plateform_code->platform_code}}</td>
                                        <td>{{$remain_amount}}</td>
                                        <td><input type="text" class="form-control form-control-sm" value="{{ $remain_amount }}" name="details[{{$key}}][amount]" required></td>
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            'use strict';
            $('#close_month').DataTable( {
                    dom: 'Bfrtip',
                        buttons: [
                            {
                                extend: 'excel',
                                title: 'Carrefour Close',
