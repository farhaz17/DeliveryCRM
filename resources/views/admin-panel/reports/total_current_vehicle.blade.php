        ...
        <div class="card text-left">
            <div class="card-body">
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    <li class="nav-item"><a class="nav-link active" id="home-basic-tab" data-toggle="tab" href="#homeBasic" role="tab" aria-controls="homeBasic" aria-selected="true">Total Current Vehicle</a></li>
                    <li class="nav-item"><a class="nav-link" id="profile-basic-tab" data-toggle="tab" href="#profileBasic" role="tab" aria-controls="profileBasic" aria-selected="false">Total Cancel Vehicle</a></li>
                    <li class="nav-item"><a class="nav-link" id="zds-basic-tab" data-toggle="tab" href="#zdsBasic" role="tab" aria-controls="zdsBasic" aria-selected="false">Vehicle in Use</a></li>
                    <li class="nav-item"><a class="nav-link" id="passport-basic-tab" data-toggle="tab" href="#passportBasic" role="tab" aria-controls="passportBasic" aria-selected="false">Free Vehicle</a></li>
...
{{--tab2--}}
                    <div class="tab-pane fade show" id="profileBasic" role="tabpanel" aria-labelledby="profile-basic-tab">
                        ...
                        <table class="table" id="datatable2"  style="width: 100%">
                            ...
                            <tbody>
                            @foreach($total_cancel_vehilce as $row)
                                <tr>
                                    <td> {{$row->id}}</td>
                                    <td> {{isset($row->plate_no)?$row->plate_no:""}}</td>
                                    <?php
                                    $cancel_date=date('d-m-Y', strtotime($row->date_and_time));
                                    ?>
                                    <td> {{$cancel_date}}</td>
                                    <td> {{$row->remarks}}</td>
                                </tr>
...
{{--                    tab3--}}
                    <div class="tab-pane fade show" id="zdsBasic" role="tabpanel" aria-labelledby="zds-basic-tab">
                        ...
                        <table class="table" id="datatable5" style="width: 100%">
                            ...
                            <tbody>
                            @foreach($vehilce_in_use as $res_bike)
                                <tr>
                                    <td>{{isset($res_bike->passport->id)?$res_bike->passport->id:""}}</td>
                                    <td>{{isset($res_bike->passport->passport_no)?$res_bike->passport->passport_no:""}}</td>
                                    <td>{{$res_bike->passport->pp_uid}}</td>
                                    <td>{{ isset($res_bike->passport->zds_code->zds_code)?$res_bike->passport->zds_code->zds_code:""}}</td>
                                    <td>{{isset($res_bike->passport->personal_info->full_name)?$res_bike->passport->personal_info->full_name:""}}</td>
                                    <td style="white-space: nowrap">
                                        {{ isset($res_bike->bike_plate_number->plate_no)?$res_bike->bike_plate_number->plate_no:""}}
                                    </td>
                                </tr>
                            @endforeach
