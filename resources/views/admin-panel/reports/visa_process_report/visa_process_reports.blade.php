        ...
        <div class="card text-left">
            <div class="card-body">
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    ...
                    <li class="nav-item"><a class="nav-link" href="{{route('visa_process_report')}}" target="_blank" class="btn btn-success m-1" type="button">View All</a></li>
                </ul>
                    ...
                    <div class="tab-pane fade show active" id="homeBasic" role="tabpanel" aria-labelledby="home-basic-tab">
                        ...
                        <table class="display table table-striped table-bordered" id="datatable1" style="width: 100%" >
                            ...
                            <tr>
                                ...
                                <th scope="col">Visa Status Status</th>
...
{{--                                <th scope="col">ID Status Status</th>--}}
    ...
    @if(isset($row->ppuid_cancel->working_status))
                                            @if($row->ppuid_cancel->working_status=='1')
                                                    Shuffle Platform
                                                    @elseif($row->ppuid_cancel->working_status=='2')
                                                    Vacation
                                                    @elseif($row->ppuid_cancel->working_status=='3')
                                                    Terminate By Platform
                                                    @elseif($row->ppuid_cancel->working_status=='4')
                                                    Terminate By Company
                                                    @elseif($row->ppuid_cancel->working_status=='5')
                                                    Accident
                                                    @elseif($row->ppuid_cancel->working_status=='6')
                                                    Absconded
                                                    @elseif($row->ppuid_cancel->working_status=='7')
                                                    Demised
                                                    @elseif($row->ppuid_cancel->working_status=='8')
                                                    Cancellation
                                                    @elseif($row->ppuid_cancel->working_statu=='9')
                                                    4PL offboard
                                                @elseif($row->ppuid_cancel->working_status=='10')
                                                    Own Visa offboard
                                             @else
                                                    None
                                                @endif
        ...
        @endif
                                       ...
                                       @if(isset($row->ppuid_cancel->visa_status))
                                                @if($row->ppuid_cancel->visa_status=='5')
                                                    Cancel
                                                @elseif($row->ppuid_cancel->visa_status=='6')
                                                    Terminated
                                                @elseif($row->ppuid_cancel->visa_status=='7')
                                                    Resign
                                                @elseif($row->ppuid_cancel->visa_status=='8')
                                                    Absconded
                                                @else
                                                    None
                                                @endif
                                       @else
                                        ...
                                        <td>
                                            @if(isset($row->ppuid_cancel->id_status))
                                                @if($row->ppuid_cancel->id_status=='1')
                                                    Active
                                                @elseif($row->ppuid_cancel->id_status=='2')
                                                    Inactive
                                                @elseif($row->ppuid_cancel->id_status=='3')
                                                    Wait Listed
                                                @elseif($row->ppuid_cancel->id_status=='4')
                                                    Hold
                                                @else
                                                    None
                                                @endif
                                    ...
                                    <td> @if(isset($row->cancel_status))
                                            @if($row->cancel_status=='0' || $row->cancel_status===null)
                                    Active
                                            @else
                                             Deactive
                                             @endif
                                ...
                                @foreach($passport as $row)
                                    <tr>
                                        ...
                                        <td>{{isset($row->sim_checkin()->telecome->account_number) ? ($row->sim_checkin()->telecome->account_number) : 'N/A'}}</td>
                                         <td>{{isset($row->agreement) ? ($row->agreement->agreement_no) : 'N/A'}}</td>
