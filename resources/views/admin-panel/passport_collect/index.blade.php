                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{route('passport_collect.update',$row->id)}}"  method="post">
                                                ...
                                                {{ method_field('PUT') }}
                                                @if($row->status == 1)
                                                    <a class="text-success mr-2 renew_btn_cls" id="{{$row->id}}" data-toggle="modal" data-id="{{$row->id}}" data-item="{{ $row->passport->id }}" data-target="#renew" type="button" href="javascript:void(0)">Transfer</a>
                                                    @if(in_array(20, Auth::user()->user_group_id) || in_array(1, Auth::user()->user_group_id))
                                                    <a class="btn btn-success mr-2 renew_btn_cls" id="{{$row->id}}" data-toggle="modal" data-id="{{$row->id}}" data-item="{{ $row->passport->id }}" data-target="#locker" type="button" href="javascript:void(0)">Send to Locker</a>
                                                    @endif
                                                    &nbsp;
                                                @elseif($row->status == 0)
                                                <div class="row">
                            ...
                            <table class="display table table-striped table-bordered" id="datatable_passport_transferred" style="width:100%;">
                                ...
                                @foreach($passports_transferred as $row)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{route('passport_collect.cancel_transfer',$row->id)}}"  method="post">
                                                {!! csrf_field() !!}
                                                {{ method_field('PUT') }}
                                                @if($row->status == 0)
                                                <div class="row">
