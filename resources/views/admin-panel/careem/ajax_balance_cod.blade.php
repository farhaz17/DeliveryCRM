...
<ul class="nav nav-tabs small" id="myTab" role="tablist">
    ...
    <li class="nav-item">
        <a class="nav-link" id="NoRiderCodsTab" data-toggle="tab" href="#NoRiderCods" role="tab" aria-controls="NoRiderCods" aria-selected="true">All No Careem / No DC Rider Cods</a>
    </li>
</ul>
            ...
            <table class="table table-sm table-striped table-bordered text-11" id="datatable">
                ...
                <tbody>
                    @foreach($riderProfile as $rider)
                        @include('admin-panel.careem.balance_cod_column')
                    @endforeach
                </tbody>
            </table>
        </div>
    ...
    <div class="modal-content">
        <form>
            ...
            <div class="modal-body">
                ...
                <input type="hidden" name="careem_upload_id" id="careem_upload_id">
...
<script>
    $('.follow_up_add_btn').click(function(){
        ...
        $('#careem_upload_id').val($(this).attr('data-careem_upload_id'))
    });
</script>
<script>
    $('.follow_up_list_btn').click(function(){
        ...
        var careem_upload_id = $(this).attr('data-careem_upload_id')
        var url = "{{ route('careem_follow_up_calls') }}";
        $.ajax({
            url,
            method: 'GET',
            data: { careem_upload_id },
...
<script>
    $('#follow_up_save_btn').click(function(){
        ...
        var careem_upload_id = $('#careem_upload_id').val();
        var feedback_id = $('#feedback_id').val();
        var remarks = $('#remarks').val();
        var _token = "{{ csrf_token() }}";
        var url = "{{ route('save_careem_followup') }}";
        $.ajax({
            url,
            method: 'POST',
            data: { _token, passport_id, careem_upload_id, feedback_id, remarks },
            success: function(response){
