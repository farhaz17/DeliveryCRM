...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active text-danger" aria-current="page">Accident Request For TeamLeader</li>
        </ol>
    </nav>
    ...
    <div class="modal fade bd-example-modal-lg" id="detail_modal"   tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form method="post" id="checkout_form" action="{{ route('save_request_teamlader_action') }}" enctype="multipart/form-data">
                    @csrf
    ...
    {{-- Rta Request Modal --}}
    <div class="modal fade bd-example-modal-lg" id="rta_modal"   tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form method="post" id="rta_form" action="{{ route('save_request_teamlader_action') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-5"></div>
                            <div class="col-7">
                                <div class="card-title">Select checkout type</div>
                                <label class="radio radio-outline-success">
                                    <input type="radio"  value="1"  name="checkout_type"  class="source_from" checked /><span>Complete Checkout</span><span class="checkmark"></span>
                                </label>
                                <label class="radio radio-outline-secondary">
                                    <input type="radio"   value="2"  name="checkout_type"  class="source_from"  /><span>Replacement</span><span class="checkmark"></span>
                                </label>
                            </div>
                        </div>
                        <div class="append_div">
                    ...
                    <input type="hidden" name="primary_id" id="rta_id">
                    <input type="hidden" name="request_rta" value="1">
                    <input type="hidden" name="request_type" id="rta_request_type">

                    <input type="hidden" name="rejected_btn_counts" id="rejected_btn_counts" value="0">
                    <div class="modal-footer" style="display: flow-root;">
                        <input type="submit" id="form_submit_button" style="display: none;">
                        <button class="btn btn-secondary " type="button" data-dismiss="modal">Close</button>
                        <button class="btn btn-danger float-right ml-2" id="rta_reject_btn" type="button">Reject</button>
                        <button class="btn btn-success float-right ml-2"  id="rta_btn" type="button">Accept</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="card text-left">
            <div class="card-body">
                ...
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    <li class="nav-item"><a class="nav-link text-10 active" id="home-basic-tab" data-toggle="tab" href="#homeBasic" role="tab" aria-controls="homeBasic" aria-selected="true">Pending Requests ( {{ isset($pending_request) ? $pending_request->count() : '' }} )</a></li>
                    <li class="nav-item"><a class="nav-link text-10 " id="home-basic-new_taken" data-toggle="tab" href="#new_taken" role="tab" aria-controls="new_taken" aria-selected="true">Accepted Requests ( {{ isset($accepted_request) ? $accepted_request->count() : '' }} )</a></li>
                    <li class="nav-item"><a class="nav-link text-10" id="profile-basic-tab" data-toggle="tab" href="#profileBasic" role="tab" aria-controls="profileBasic" aria-selected="false">Rejected Requests( {{ isset($rejected_request) ? $rejected_request->count() : '' }} )</a></li>
                </ul>
                    ...
                    <div class="tab-pane fade show active" id="homeBasic" role="tabpanel" aria-labelledby="home-basic-tab">
                        ...
                        <div class="table-responsive">
                            <table class="display table table-striped table-bordered table-sm text-10" id="datatable_not_employee">
                                ...
                                <tbody>
                                    @foreach($pending_request  as $ab)
                                    @if(in_array($ab->plateform, Auth::user()->user_platform_id) || in_array(1, Auth::user()->user_group_id))
                                    <tr>
                                        <td>{{ $ab->id }}</td>
                                        ...
                                        <td>
                                            @if ($ab->rta_request == 1)
                                            <a class="text-primary mr-2 rta_cls" id="{{ $ab->id }}" data-status="0" href="javascript:void(0)"><i class="nav-icon i-eye font-weight-bold"></i></a>
                                            @else
...
@section('js')
    ...
    <script>
        $('body').on('click','.view_cls',function () {
            ...
            $.ajax({
                url: "{{ route('get_accident_request_ajax') }}",
                method: 'get',
                data: {id_primary: ids},
                success: function(response) {
                    $(".append_div").empty();
        ...
        $('body').on('click','.rta_cls',function () {

        $("#rta_modal").modal('show');
        var ids = $(this).attr('id');
        $("#rta_id").val(ids);
        $("#rejected_btn_counts").val("0");
        var now_status = $(this).attr('data-status');
        ...
        $("#rta_btn").click(function(){
            $("#rta_request_type").val("1");
            $("#rta_form").submit();
        });

        $("#rta_reject_btn").click(function(){
            $("#rta_request_type").val("2");
            $(".reject_resaon_div").show(300);
            $("#rta_form").submit();

        });

        $("#checkout_form, #rta_form").submit(function(e) {
