...
@section('content')
    ...
    <div class="col-md-12 mb-3">
        <div class="card text-left">
            <div class="card-body">
                <ul class="nav nav-tabs" id="myTab" role="tablist">
                    ...
                    <li class="nav-item"><a class="nav-link" id="profile-basic-tab" data-toggle="tab" href="#solved" role="tab" aria-controls="solved" aria-selected="false">Transferred ({{ $passports_transferred->count() }})</a></li>
                </ul>
                            ...
                            <table class="display table table-striped table-bordered" id="datatable_passport">
                                ...
                                @foreach($passports as $row)
                                    ...
                                    <tr>
                                        ...
                                        <td> {{$row->reason}}</td>
                                        <td> {{$row->remarks}}</td>
                                        <td> {{isset($row->user) ? $row->user->name: 'Locker'}} </td>
                                        <td class="tranbsfer_btn">
                                            <form action="{{route('request_passport.update_transfer',$row->id)}}"  method="post">
                                                {!! csrf_field() !!}
                                                {{ method_field('PUT') }}
                                                @if($row->status == 1)
                                                    <a class="text-success mr-2 renew_btn_cls" id="{{$row->id}}" data-toggle="modal" data-id="{{$row->id}}" data-item="{{ $row->passport->id }}" data-obj="{{ $row->user_request }}" data-target="#renew" type="button" href="javascript:void(0)">Transfer</a>
                                                    @if($row->user_request == 1)
                                                    {{-- <a class="btn btn-success mr-2 renew_btn_cls" id="{{$row->id}}" data-toggle="modal" data-id="{{$row->id}}" data-item="{{ $row->passport->id }}" data-target="#to_manager" type="button" href="javascript:void(0)">Transfer to Passport Manager</a> --}}
                                                    @else
                                                    <a class="btn btn-success mr-2 renew_btn_cls" id="{{$row->id}}" data-toggle="modal" data-id="{{$row->id}}" data-item="{{ $row->passport->id }}"   data-target="#rider" type="button" href="javascript:void(0)">Transfer to Rider</a>
                                                    @endif
                                                    @if(in_array(20, Auth::user()->user_group_id))
                                                        <a class="btn btn-success mr-2 renew_btn_cls m-2" id="{{$row->id}}" data-toggle="modal" data-id="{{$row->id}}" data-item="{{ $row->passport->id }}" data-target="#locker" type="button" href="javascript:void(0)">Send to Locker</a>
                                                    @endif
                                                    &nbsp;
                                                @elseif($row->status == 0)
                                                <div class="row">
                            ...
                            <table class="display table table-striped table-bordered" id="datatable_passport_transferred" style="width:100%;">
                                ...
                                @foreach($passports_transferred as $row)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form action="{{route('request_passport.cancel_transfer',$row->id)}}"  method="post">
                                                {!! csrf_field() !!}
                                                {{ method_field('PUT') }}
                                                @if($row->status == 0)
                                                <div class="row">
                                                    Transfer Pending <span class="ml-1"> <input type="submit" name="accept" class="btn btn-danger ml-2 mr-2" value="Cancel Transfer"></span>
                                                </div>
                                                @elseif($row->status == 2)
                                                    Rejected
     ...
     <div class="modal fade" id="renew" role="dialog" aria-labelledby="verifyModalContent" aria-hidden="true">
            ...
            <div class="row">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="renew">
                            <form   method="post" action="{{ route('request_passport.post_transfer') }}">
                                ...
                                <input type="hidden" id="user_request" name="user_request" />
                                <div class="col-md-12 form-group mb-3">
        ...
        <!---------Transfer Passport Manager---------->
     <div class="modal fade" id="to_manager" role="dialog" aria-labelledby="verifyModalContent" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="row">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="renew">
                            <form   method="post" action="{{ route('request_passport.transfer_to_manager') }}">
        ...
        <!---------Send To Rider---------->
        <div class="modal fade" id="rider" tabindex="-1" role="dialog" aria-labelledby="verifyModalContent" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="row">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="renew">
                                <form   method="post" action="{{ route('request_passport.rider') }}">
                                    ...
                                    <div class="col-md-12 form-group mb-3">
                                        ...
                                        <select class="form-control" name="reason">
                                            ...
                                            <option value="5">Passport Renewal</option>
                                            <option value="6">Bank/Other Services</option>
                                        </select>
                                    </div>

                                    <div class="modal-footer">
                                        <div class="col-md-12">
                                            <input type="submit" name="bulk_transfer" class="btn btn-primary" value="Transfer">
                                        </div>
                                    </div>
                                </form>
                            </div>
                ...
                <div class="row">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="renew">
                                <form   method="post" action="{{ route('request_passport.locker') }}">
...
<script>
    $(document).on("click", ".renew_btn_cls", function () {
        ...
        var user_request = $(this).attr('data-obj');
        // console.log(pid);
        $("input[name='passport_id']").val(itemid);
        $("input[name='collection_id']").val(pid);
        $("input[name='user_request']").val(user_request);
    });
