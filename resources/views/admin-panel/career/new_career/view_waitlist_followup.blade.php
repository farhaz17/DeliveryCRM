        ...
        <div class="card-body">
             ...
             <div class="wait_list">
                <ul class="nav nav-tabs" id="myTab" role="tablist" style="margin-top: 10px;">
                    <li class="nav-item">
                        <a class="nav-link active" id="wTodayTab" data-toggle="tab" href="#wToday" role="tab" aria-controls="wToday" aria-selected="true">Today ({{$career_counts}})
                        </a>
                    </li>
                    @foreach($followup_waits as $follow)
                    <li class="nav-item">
                        <a class="nav-link followups" id="{{ $follow->name }}Tab" data-ids="{{ $follow->id }}" data-toggle="tab" href="#Intersted" role="tab" aria-controls="Intersted" aria-selected="false">{{ $follow->name }} ({{$follow->count ?? 0}})
                        </a>
                    </li>
                    @endforeach
                </ul>
                            ...
                            <table class="display table table-sm table-striped text-10 table-bordered" id="wTodayTable" style="width: 100%;">
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" class="filtering_source_from">Visa Status</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ isset($source_type_array[$todays->career->source_type]) ? $source_type_array[$todays->career->source_type] : 'N/A' }}</td>
                                        <td>
                                            <a class="text-primary mr-2 follow_up" data-category="2" data-today="1" id="{{ $todays->career->id  }}" href="javascript:void(0)"><i class="nav-icon i-Pen-2 font-weight-bold"></i></a>
                                    ...
                                    <tr>
                                        ...
                                        <td>N/A</td>
                                        <td>
                                            <a class="text-primary mr-2 rejoin_follow_up" data-category="2" data-today="1" id="{{ $todays->passport_id  }}" href="javascript:void(0)"><i class="nav-icon i-Pen-2 font-weight-bold"></i></a>
                                            ...
                                            <a class="text-danger mr-2 rejection_rejoin" id="{{ $todays->id  }}" href="javascript:void(0)"><i class="nav-icon i-Close font-weight-bold"></i></a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                    ...
                    <div class="row">
                        <div class="col-md-12 form-group">
                            ...
                            <select class="form-control follow_up_status" id="follow_up_status" name="follow_up_status" required>
                                ...
                                @foreach($followup_wait as $follow)
    ...
    <div class="modal fade bd-example-modal-sm"  id="reject_remarks_modal"tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle-1" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <form action="{{ route('waitlist_reject_save') }}" method="POST">
                    @csrf
     ...
     <div class="modal fade bd-example-modal-sm"  id="rejoin_reject_remarks_modal"tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle-1" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalCenterTitle-1">Warning</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                </div>
                <form action="{{ route('waitlist_rejoin_reject_save') }}" method="POST">
                    @csrf
    ...
    <div class="modal fade bd-example-modals-lg" id="passport_detail_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modals-lg">
            ...
            @section('js')
                ...
                <script>
                    $(document).ready(function () {
                        ...
                        retrieve: true,
                        ajax:{
                            url : "{{route('ajax_waitlist_follow_up')}}",
                            data:{id: id},
                        },
                        columns: [
                            { data: 'career_name' },
            ...
            $.ajax({
              ...
              success: function(response) {
                  ...
                  for(var i=0; i<len; i++){
                    ...
                    data = `<div class="message flex-grow-1">
                            <div class="d-flex">
                                <span class="mb-0 text-title text-12 flex-grow-1">${response[i].name}</span>
                                <span class="career_remark">${response[i].note_added_date}</span>
                            </div>
                                <p class="m-0">${response[i].remarks}</p>
                                <span class="m-0">(${$career_category}  ${$followup})</span>
            ...
            <script>
                $(document).ready(function () {
                    'use strict';
                    $('#wInterstedTable, #wCallMeTomorrowTable, #wNoResponseTable,#wNotInterestedTable').DataTable( {
                        "aaSorting": [[0, 'desc']],
                        "pageLength": 10,
                        "columnDefs": [
