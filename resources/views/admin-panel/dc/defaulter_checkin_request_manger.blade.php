...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Checkin Request List Of Defaulter Rider</li>
        </ol>
    </nav>
...
@section('js')
    ...
    <script>
        function make_table(table_name,status,plat_id) {
            ...
            $.ajax({
                url: "{{ route('get_checkin_request_render_defaulter_manager') }}",
                method: 'GET',
    ...
    <script>
        $('body').on('click','.view_cls',function () {
            ...
            $.ajax({
                url: "{{ route('get_checkin_request_ajax_defaulter_manager') }}",
                method: 'get',
                data: {id_primary: ids},
                success: function(response) {
                    $(".append_div").empty();
