...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Checkout Request List Of Defaulter Rider</li>
    </ol>
</nav>
        ...
        <div class="modal-content">
            <form method="post" id="checkout_form" action="{{ route('dc_request.update',1) }}" enctype="multipart/form-data">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalCenterTitle">Checkout Request Detail Of Defaulter Rider</h5>
...
{{-- <div class="card-body">
    <button class="btn btn-primary m-1 cls_type_riders" id="manger_send_rider" type="button">Normal Rider Requests</button>
    <button class="btn btn-outline-primary m-1 cls_type_riders" id="defaulter_manger_send_rider" type="button">Defaulter riders Requests</button>
</div> --}}
    ...
    <div class="card text-left">
        <div class="card-body">
            ...
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item"><a class="nav-link text-10 active" id="home-basic-tab" data-toggle="tab" href="#homeBasic" role="tab" aria-controls="homeBasic" aria-selected="true">Pending Requests ({{ isset($requests) ? $requests->count() :'0' }})</a></li>
            ...
            <div class="card-body">
                ...
                <div class="tab-content" id="defaulter_myTabContent">
                    ...
                    <div class="tab-pane fade show " id="defaulter_new_taken" role="tabpanel" aria-labelledby="defaulter_home-basic-new_taken">
                        ...
                        <div class="table-responsive">
                            <table class="display table table-striped table-bordered table-sm text-10" id="defaulter_datatable_accepted">
                                <thead >
                                <tr>
                                    <th scope="col">#</th>
                                    <th scope="col">Rider Name</th>
...
<script>
    $(".cls_type_riders").click(function(){
        ...
        if(ids=="defaulter_manger_send_rider"){
            ...
            $(".display").css("width","100%");
               ...
               make_table_defaulter("defaulter_pending_datatable","0");
    ...
    <script>
        ...
        $('body').on('shown.bs.tab', 'a[data-toggle="tab"]', function (e) {
            ...
            }else if(currentTab=="defaulter_home-basic-tab"){

                make_table_defaulter("defaulter_pending_datatable","0");
                var table = $('#defaulter_pending_datatable').DataTable();
                $('#container').css( 'display', 'block' );
                table.columns.adjust().draw();

            }else if(currentTab=="defaulter_home-basic-new_taken"){

                make_table_defaulter("defaulter_datatable_accepted","1");
                var table = $('#defaulter_datatable_accepted').DataTable();
                $('#container').css( 'display', 'block' );
                table.columns.adjust().draw();

            }else if(currentTab=="defaulter_profile-basic-tab"){

                make_table_defaulter("defaulter_rejected_datatable","2");
                var table = $('#defaulter_rejected_datatable').DataTable();
                $('#container').css( 'display', 'block' );
                table.columns.adjust().draw();
...
<script>
    function make_table(table_name,status,plat_id) {
        ...
        $.ajax({
            url: "{{ route('get_checkout_request_render_defaulter_manager') }}",
            method: 'GET',
    ...
    function make_table_defaulter(table_name,status){
                $.ajax({
                    url: "{{ route('get_defaulter_checkout_request_render') }}",
                    method: 'GET',
                    data: {request_type:status},
                    success: function(response) {
